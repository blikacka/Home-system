...
class OdoGraph extends Graph {
	...
	function Stroke($aStrokeFileName = "") {
		...
		if ($this->doshadow) {
			$rm += $this->shadow_width;
		}
		if ($this->doshadow) {
			$bm += $this->shadow_width;
		}
		...
		$this->caption->Center($this->img->left_margin, $this->img->width - $this->img->right_margin, $captiony);
		...
		switch ($this->iStyle) {
			...
			case NEEDLE_STYLE_STRAIGHT: // Straight - two widths
				...
				$p = [
					...
					$yc + $ind - $yadj,
					$xc + $r,
					$yc + $ind + ($this->iWeight - 2 * $ind) - $yadj,
					$xc + $r * $this->iStyleParameter1,
					$yc + $ind + ($this->iWeight - 2 * $ind) - $yadj,
					$xc + $r * $this->iStyleParameter1,
					$yc + $this->iWeight - $yadj,
			...
			case NEEDLE_STYLE_ENDARROW: // With end arrow
				...
				$p = [
					...
					$yc - $yadj,
					$xc + $r + $arrow_length,
					$yc + $arrow_width + $this->iWeight / 2 - $yadj,
					$xc + $r,
					$yc + 2 * $arrow_width + $this->iWeight - $yadj,
					$xc + $r,
					$yc + $arrow_width + $this->iWeight - $yadj,
					$xc,
					$yc + $arrow_width + $this->iWeight - $yadj
				];
	...
	function Stroke($img, $aOdometer) {
		...
		for ($i = 0; $i <= $n; ++$i) {
			...
			if ($i % $this->iLabelInterval == 0) {
				$p = [
					$aOdometer->xc + round($r * cos($a) * 0.99),
				...
				$p = [
					$aOdometer->xc + round($aOdometer->iRadius * cos($a)),
					$aOdometer->yc - round($r * sin($a)),
...
class Odometer {
	...
	function StrokeFascia($img) {
		...
		$this->FilledCircle($img, $this->xc, $this->yc, $this->iCenterAreaWidth * $this->iRadius, $this->iFillColor);
	...
	function Stroke($graph) {
		...
		$this->xc = $this->xc <= 1 ? floor($img->plotwidth * $this->xc) : $this->xc;

		// We only do automatic adjust of the Y-coordinate if the position
		// is given as fractions
		$doautoadjust = ($this->yc < 1) ? 1 : 0;

		$this->yc = $this->yc <= 1 ? floor($img->plotheight * (1 - $this->yc)) : $this->yc;
		if ($this->iStyle == ODO_HALF) {
			$this->yc -= $this->iBorderWidth + $this->iMargin;
			$this->iRadius = $this->iRadius <= 1 ? min(floor($this->iRadius * ($img->plotwidth / 2)), floor($this->iRadius * $img->plotheight)) - 2 * $this->iMargin : $this->iRadius;
			$this->iRadius -= $this->iBorderWidth;
		} else {
			$this->iRadius = $this->iRadius <= 1 ? floor($this->iRadius * min($img->plotwidth, $img->plotheight)) - $this->iMargin : $this->iRadius;
		}
		...
		if ($this->iStyle == ODO_HALF) {
			...
			$this->caption->Align("center", "top");
			$this->caption->Stroke($img, $this->xc, $this->yc + 1 + $this->iRadius + $this->caption->margin);
		}
